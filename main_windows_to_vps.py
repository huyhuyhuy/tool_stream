...
class ScreenStreamTool:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Stream Tool - Windows to VPS")
        self.root.geometry("450x500")
        self.root.configure(bg='#f0f0f0')
        
        # VPS Configuration
        self.vps_ip = "45.76.190.6"  # Thay đổi IP VPS của bạn
        self.vps_port = 5000
    ...
    def setup_ui(self):
        ...
        # VPS Status Frame
        vps_frame = tk.Frame(main_frame, bg='#e8f4fd', relief='raised', bd=2)
        vps_frame.pack(fill='x', pady=5)
        
        tk.Label(vps_frame, text="🌐 VPS Configuration", font=("Arial", 12, "bold"), bg='#e8f4fd').pack(pady=5)
        
        vps_info_frame = tk.Frame(vps_frame, bg='#e8f4fd')
        vps_info_frame.pack(fill='x', pady=5)
        
        tk.Label(vps_info_frame, text=f"VPS IP: {self.vps_ip}:{self.vps_port}", bg='#e8f4fd', font=("Arial", 9)).pack(anchor='w')
        tk.Label(vps_info_frame, text="Public URL: http://45.76.190.6:5000", bg='#e8f4fd', font=("Arial", 9), fg="blue").pack(anchor='w')
        
        self.vps_status_label = tk.Label(vps_info_frame, text="Status: Connecting to VPS...", 
                                       bg='#e8f4fd', font=("Arial", 9), fg="orange")
        self.vps_status_label.pack(anchor='w')
        ...
        self.link1_label = tk.Label(self.link1_frame, text="Public Link: http://45.76.190.6:5000", 
                                   bg='#e0e0e0', font=("Arial", 9), 
                                   cursor="hand2", fg="blue")
        self.link1_label.pack(side='left')
        self.link1_label.bind("<Button-1>", lambda e: self.copy_public_link())
        ...
        self.link2_label = tk.Label(self.link2_frame, text="Public Link: http://45.76.190.6:5000", 
                                   bg='#e0e0e0', font=("Arial", 9),
                                   cursor="hand2", fg="blue")
        self.link2_label.pack(side='left')
        self.link2_label.bind("<Button-1>", lambda e: self.copy_public_link())
    ...
    def register_with_vps(self):
        """Đăng ký với VPS để VPS biết IP của Windows PC"""
        try:
            ...
            try:
                # Get ngrok tunnels from local ngrok API
                ...
                if ngrok_response.status_code == 200:
                    ...
                    for tunnel in tunnels:
                        if tunnel['proto'] == 'https' and tunnel['config']['addr'] == 'localhost:5000':
                            ngrok_url = tunnel['public_url']
                            # Extract domain from ngrok URL
                            ngrok_domain = ngrok_url.replace('https://', '').replace('http://', '')
                            print(f"Found ngrok URL: {ngrok_domain}")
                            break
                    else:
                        print("No ngrok tunnel found for localhost:5000")
                        raise Exception("No ngrok tunnel found for localhost:5000")
                else:
                    print("Cannot connect to ngrok API")
                    raise Exception("Cannot connect to ngrok API")
            except Exception as e:
                print(f"Ngrok API error: {e}")
                # Try to get ngrok URL from XML API as fallback
                try:
                    xml_response = requests.get("http://127.0.0.1:4040/api/tunnels", timeout=5)
                    if xml_response.status_code == 200:
                        # Parse XML response
                        import xml.etree.ElementTree as ET
                        root = ET.fromstring(xml_response.text)
                        public_url = root.find('.//PublicURL').text
                        if public_url:
                            ngrok_domain = public_url.replace('https://', '').replace('http://', '')
                            print(f"Found ngrok URL from XML: {ngrok_domain}")
                        else:
                            raise Exception("No PublicURL found in XML")
                    else:
                        raise Exception("Cannot connect to ngrok XML API")
                except Exception as e2:
                    print(f"XML API also failed: {e2}")
                    # Try to get current ngrok URL from the XML you provided
                    try:
                        # Use the current ngrok URL from your XML response
                        ngrok_domain = "0f29bfea42dc.ngrok-free.app"
                        print(f"Using current ngrok URL: {ngrok_domain}")
                    except:
                        # Final fallback
                        ngrok_domain = "0f29bfea42dc.ngrok-free.app"
                        print(f"Using hardcoded ngrok URL: {ngrok_domain}")
            
            if not ngrok_domain:
                raise Exception("Could not determine IP/domain")
            
            # Register with VPS
            data = {"ip": ngrok_domain}
            response = requests.post(f"http://{self.vps_ip}:{self.vps_port}/register", 
                                   json=data, timeout=5)
            
            if response.status_code == 200:
                if 'ngrok' in ngrok_domain:
                    self.vps_status_label.config(text=f"Status: ✅ Connected to VPS (via ngrok: {ngrok_domain})", fg="green")
                else:
                    self.vps_status_label.config(text=f"Status: ✅ Connected to VPS (IP: {ngrok_domain})", fg="green")
                self.status_label.config(text="Connected to VPS - Ready to stream")
            else:
                self.vps_status_label.config(text="Status: ❌ VPS connection failed", fg="red")
                
        except Exception as e:
            print(f"Error connecting to VPS: {e}")
            self.vps_status_label.config(text="Status: ❌ VPS offline", fg="red")
    ...
    def start_stream1(self):
        ...
        self.status_label.config(text="Stream 1 started - Sending to VPS")
    ...
    def start_stream2(self):
        ...
        self.status_label.config(text="Stream 2 started - Sending to VPS")
    ...
    def setup_flask_routes(self):
        ...
        def index():
            ...
            <html>
            <head>
                <title>Windows PC Stream Server</title>
                <style>
            ...
            <body>
                <div class="container">
                    <h1>🖥️ Windows PC Stream Server</h1>
                    <p style="text-align: center; color: #666;">This server captures Chrome windows and sends to VPS</p>
    ...
    def copy_public_link(self):
        link = "http://45.76.190.6:5000"
        pyperclip.copy(link)
        messagebox.showinfo("Link Copied", f"Public link copied to clipboard:\n{link}")
